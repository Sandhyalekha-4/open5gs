...
enb_ue_t *enb_ue_add(mme_enb_t *enb, uint32_t enb_ue_s1ap_id)
{
    ...
    ogs_info("IMSI [%s] and before adding eNB ue count [%d]",  MME_UE_HAVE_IMSI(mme_ue) ? mme_ue->imsi_bcd : "Unknown", num_of_enb_ue);
    stats_add_enb_ue();
...
void enb_ue_remove(enb_ue_t *enb_ue)
{
    ...
    ogs_pool_id_free(&enb_ue_pool, enb_ue);
    mme_ue_t  *mme_ue = mme_ue_find_by_id(enb_ue->mme_ue_id);
    ogs_info("IMSI [%s] and before removing eNB ue count [%d]",  MME_UE_HAVE_IMSI(mme_ue) ? mme_ue->imsi_bcd : "Unknown", num_of_enb_ue);
    stats_remove_enb_ue();
}
...
int mme_ue_set_imsi(mme_ue_t *mme_ue, char *imsi_bcd)
{
     ...
     */
            ...
            if (!old_sgw_ue) {
                ...
                sgw_ue->sgw_s11_teid = OGS_INVALID_POOL_ID;
               } else {
